    ...
    if request.user.is_authenticated:
        ...
        if user_id:
            try:
                ...
                expense_previous=Expense.objects.filter(user_id=user, spent_date__range=(first_day_previous_month, last_day_previous_month))
                income_previous=Income.objects.filter(user_id=user,spent_date__range=(first_day_previous_month, last_day_previous_month))
                savings_previous=Savings.objects.filter(user_id=user,spent_date__range=(first_day_previous_month, last_day_previous_month))
                

                expense_today=Expense.objects.filter(user_id=user,spent_date__range=(first_day_this_month, today))
                income_today=Income.objects.filter(user_id=user,spent_date__range=(first_day_this_month, today))
                savings_today=Savings.objects.filter(user_id=user,spent_date__range=(first_day_this_month, today))
                ...
                if expense_total_previous==0 and expense_total_today==0:
                    ...
                    if percentage<0:
                        expense_percentage=f"{-percentage:.2f}% less than previous month"
                    elif percentage>0:
                        expense_percentage=f"{percentage:.2f}% more than previous month"
                    else:
                ...
                if income_total_previous==0 and income_total_today==0:
                    ...
                    if percentage<0:
                        income_percentage=f"{-percentage:.2f}% less than previous month"
                    elif percentage>0:
                        income_percentage=f"{percentage:.2f}% more than previous month"
                    else:
                ...
                if savings_total_previous==0 and savings_total_today==0:
                    ...
                    if percentage<0:
                        savings_percentage=f"{-percentage:.2f}% less than previous month"
                    elif percentage>0:
                        savings_percentage=f"{percentage:.2f}% more than previous month"
                    else:
                ...
                if balance_previous==0 and balance_today==0:
                    ...
                    balance_percentage="No balance this month"
                    ...
                    if percentage<0:
                        balance_percentage=f"{-percentage:.2f}% less than previous month"
                    elif percentage>0:
                        balance_percentage=f"{percentage:.2f}% more than previous month"
                    else:
                        balance_percentage="Same as last month"
                
                recent_expense=Expense.objects.filter(user_id=user).order_by('-spent_date')[:10]
                recent_income=Income.objects.filter(user_id=user).order_by('-spent_date')[:10]


                expense_data=[
                    {"amount":f"- रु {e.amount}", "category":e.category,"description":e.description, "spent_date":e.spent_date}
                    for e in recent_expense
                ]

                income_data=[
                    {"amount":f"+ रु {i.amount}", "category":"Income","description":i.description, "spent_date":i.spent_date}
                ...
                return render(request, 'index.html', context={"username":user.username,"email":user.email,"first_name":user.first_name,"last_name":user.last_name,"expense":expense_total_today, "income":income_total_today, "savings":savings_total_today, "balance":balance_today, "expense_percentage": expense_percentage, "income_percentage":income_percentage, "savings_percentage":savings_percentage, "balance_percentage":balance_percentage, "transaction":transaction})
            except User.DoesNotExist:
                return redirect('Login')    
    return redirect('Login')     


def TransactionView(request:HttpRequest):
    if request.user.is_authenticated:
        user_id=request.session.get('user_id')
        if user_id:
            user=User.objects.get(id=user_id)
            expense=Expense.objects.filter(user_id=user_id)
            income=Income.objects.filter(user_id=user_id)
            ...
            total_expense=f"- रु {total_expense}"
            total_income=f"+ रु {total_income}"
            total_savings=f"+ रु {total_savings}"

            if balance<0:
                balance=f"- रु {balance}"
            balance=f"+ रु {balance}"


            recent_expense=Expense.objects.filter(user_id=user_id).order_by('-spent_date')
            recent_income=Income.objects.filter(user_id=user_id).order_by('-spent_date')

            expense_data=[
                {"id":e.id, "amount":f"- रु {e.amount}", "category":e.category, "description":e.description, "spent_date":e.spent_date}
                for e in recent_expense
            ]
            income_data=[
                {"id":i.id, "amount":f"+ रु {i.amount}", "category":"Income", "description":i.description, "spent_date":i.spent_date}
                for i in recent_income
            ]
            ...
            return render(request, 'transactions.html', context={"username":user.username, "email":user.email,"first_name":user.first_name, "last_name":user.last_name, "expense":total_expense, "income":total_income, "savings":total_savings, "balance":balance, "transaction":transaction})
...
def EditExpenseView(request:HttpRequest, pk):
    if request.user.is_authenticated:
        form_obj=Expense.objects.get(id=pk)
        form=ExpenseForm(instance=form_obj)
        return render(request, 'expense.html', context={"form":form})